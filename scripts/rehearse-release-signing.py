...
PASS = "EPHEMERAL_RELEASE_SIGNING_REHEARSAL_PASS"
BLOCKED = "EPHEMERAL_RELEASE_SIGNING_REHEARSAL_BLOCKED"
...
def verify_unsigned_primaries(
    repository: Path, manifest: dict[str, object]
    ...
    if len(records) != EXPECTED_PRIMARY_COUNT:
        raise ValueError(
            f"PRIMARY_MANIFEST_MISMATCH:expected-{EXPECTED_PRIMARY_COUNT}:actual-{len(records)}"
        )
    primaries: list[Path] = []
    for record in records:
        ...
        if primary.with_name(primary.name + ".asc").exists():
            raise ValueError(f"UNEXPECTED_EXISTING_SIGNATURE:{relative.as_posix()}")
        primaries.append(primary)
    return primaries, records
...
def rehearse(repository: Path, manifest_path: Path, output: Path) -> dict[str, object]:
    ...
    primaries, records = verify_unsigned_primaries(repository, manifest)
    if output.exists():
    ...
    verified: list[str] = []
    test_fingerprint = ""
    with tempfile.TemporaryDirectory(prefix="allow-experimental-m7b-gnupg.") as temporary:
        home = Path(temporary)
        home.chmod(0o700)
        generated = run(
            [
                ...
                "",
                "--quick-generate-key",
                "Allow Experimental Ephemeral Test Signer <allow-experimental-test@example.invalid>",
                "ed25519",
                "sign",
                "0",
            ]
    ...
    if primary_manifest_digest(records) != manifest["primary_manifest_sha256"]:
        raise RuntimeError("PRIMARY_MANIFEST_CHANGED_DURING_SIGNING")

    bundle = output / "allow-experimental-central-bundle-EPHEMERAL-TEST-ONLY.zip"
    repeat = output / ".bundle-repeat.zip"
    write_deterministic_zip(signed_repository, bundle)
    write_deterministic_zip(signed_repository, repeat)
    deterministic = digest(bundle) == digest(repeat)
    repeat.unlink()
    if not deterministic:
        raise RuntimeError("BUNDLE_SERIALIZATION_NONDETERMINISTIC")

    proof: dict[str, object] = {
        "schema": "allow-experimental-ephemeral-signing-proof-v1",
        ...
        "bundle": {
            ...
            "deterministic_serialization_verified": deterministic,
        },
        "credential_boundary": {
            "central_token_read": False,
            "central_token_used": False,
            "real_release_signing_key_used": False,
        },
        "remote_actions": "NONE",
    }
